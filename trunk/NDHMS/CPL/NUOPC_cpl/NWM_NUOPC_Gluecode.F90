...
#define METHOD "NWM_ReachStreamCreate"
  ...
  ! uniform, rectilinear, curvilinear, unstructure
  !
  ...
  ! by one-dimensional latitude and longitude coordinates ([*]) will be called
  ! rectilinear; grids where the coordinates are two-dimensional ([*][*]) will
  ! be called curvilinear; and, grids in which the grid coordinates require a
  ! list of nodes (connectivity information) will be called unstructured.
  !
  ! For smoothly varying variables, bilinear interpolation is adequate for many
  ! applications. Most geophysical variables (eg., temperature) are highly
  ! spatially correlated at all time scales.
  ! 
  ...
  ! spatial correlation at high frequency sampling (eg., 3-hourly) but on monthly, 
  ! seasonal or annual mean time scales may be 'reasonably' smooth is space.
  !
  ! Computing the curl of the wind stress requires that highly accurate derivatives 
  ! be computed. In this case, patch interpolation would be appropriate. 
  !
  ! In other instances, there may be physical requirements (e.g., conservation of
  ...
  ! Deriving a new grid for categorical data (eg, land surface type) would be best 
  ! accomplished with a nearest neighbor algorithm because interpolating different 
  ...
  !
  ! Extrapolation: In some cases, a regridding algorithm may extrapolate rather than
  ! interpolate. A common example is extrapolating vertical profiles in mountain
  ...
  ! acceptable when extrapolating temperature or geopotential via the hydrostatic
  ! equation. However, generally, any extrapolated values should be used and
...
#undef METHOD
#define METHOD "NWM_Regrid"

  subroutine NWM_ReGrid(did,srcField,dstField, rc)
    ! ARGUMENTS
    integer, intent(in)         :: did
